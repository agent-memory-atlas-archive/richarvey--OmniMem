...
## [6.7.2] - Unreleased
### Fixed
- **The Test Coverage job failed on fastmcp 4 / mcp 2** (#37): with no ceiling on fastmcp, a fresh CI install resolved fastmcp 4.0.3 and mcp 2.2.0, which production already runs without trouble. The one casualty was a test: mcp 2 makes `client_id` a required string on `OAuthClientInformationFull` (its registration handler assigns a uuid before calling the provider), so `test_register_and_get` could no longer build a client with `client_id=None`. Real registrations always arrive with an id, so nothing in the server was affected. The test now registers a client the way mcp 2 delivers one, with its id already assigned, and a second test reaches the provider's generate-if-missing fallback through `model_copy`, since mcp 2 also strips an empty `client_id` at parse time. Requirements now declare the versions production runs as the floor, `fastmcp>=4.0.3,<5` and `mcp>=2.2.0,<3`, with mcp listed explicitly because fastmcp doesn't declare it, and a ceiling at the next major so the next breaking release is adopted deliberately
...
- **The floor is deliberately lower than the issue proposed, and the band above it is marked rather than cut.** The 0.4 suggested on #30 was read off `recall()`'s reported scores, which are adjusted — 1.133 is not a reachable cosine — while the floor gates the raw similarity, so the cliff did not transfer between the two scales. Measured with the shipped ONNX embedder over query/memory pairs written in this repo's own style, true positives run from 0.189 ("why do we get a 421 misdirected request" against the FastMCP Host/Origin write-up) to 0.633, and false positives from -0.06 to 0.242 ("search index drift" against the responsive-tables note). They overlap, so no single cut is both safe and useful: 0.4 kept only 6 of 8 correct answers, which a user experiences as recall forgetting something they know is stored, with nothing on screen to say a filter fired. So the floor drops only what is unambiguously noise, and `RECALL_WEAK_SCORE` (default `0.35`) flags everything between the two as `weak_match: true`. The agent instructions tell it to read a weak match but not build on it and not to go looking for a connection — which is the actual harm #30 described, and it was never the row count. Two mechanisms worth knowing before retuning: the model truncates at 256 tokens while `remember()` accepts 50,000 characters un-chunked, so a long write-up embeds as its first 256 tokens; and a short keyword query against long prose scores low by construction
- **The floor gates the raw similarity, not the adjusted score**, and that distinction is the design rather than an implementation detail. Raw similarity is the only number in the pipeline that measures whether a memory is about the same subject; the multipliers stacked on top of it encode policy — this is old, this approach was abandoned, this is a derived fact whose verbatim source should outrank it — and policy belongs in the ranking, not in the decision to show a memory at all. Gating on the adjusted score would have silently hidden the two classes deliberately scored down: extracted facts at surface 0.5 (issue #20) and abandoned experiences at weight 0.1, both of which are exactly what should still come back when nothing better matches. Abandoned-approach warnings and reinstate candidates are exempt outright — the first fires on a keyword scan before the query is embedded and the second matches on its own hints, so neither earned its place on a similarity it could be judged against
- **`find_skills()` returned every skill regardless of score** (#31): `find_skills("python")` on a store with no Python skill returned all three, led by `preferences-local` at 0.2341 and ending with a 0.0013 match that is indistinguishable from noise. The documented flow is find, then `get_skill()` on the winner, so an agent following it loads a preferences skill for Python work — and a skill loads whole, so a wrong pick costs far more context than a stray recall hit. `SKILL_MIN_SCORE` (default `0.25`) now gates the semantic pass, and an empty list is the answer when nothing covers the work. Exact domain hits are identity matches and are never gated
- **The skill floor is its own number because it measures a different thing.** A skill's discovery vector is its short name-and-description summary of what the skill is for, matched against a query about the work at hand — not a query against a memory's raw content — and unlike recall's, that distribution separates cleanly, which is why a hard cut is defensible here and only a conservative one is defensible there. Measured against three compiled skills on a live store: true positives at 1.0 (`"opentofu"`, exact domain), 0.6189 (`"accessibility"` → wcag), 0.4735 (`"what git forge and commit message style"` → preferences) and 0.3042 (`"a modal dialog that works with a keyboard and screen reader"` → wcag); highest false positive 0.2341 (`"python"` → preferences), next 0.2153, everything else at or below 0.19. Lowest true positive 0.3042 against highest false positive 0.2341 puts 0.25 in the gap with about 0.05 either side. The verbose query scoring lowest of the true positives is the pattern to keep in mind if this is ever retuned: length dilutes cosine similarity, so the floor has to clear the worst honest query rather than the best one
- **Each `find_skills()` entry carries a `confidence`** of `high` (an exact domain hit, or a semantic score at or above 0.45) or `low`, and a result set that is entirely low says so. A bare ranked list gave a caller no way to tell a good winner from the least-bad of a weak field, which is the specific failure in #31
- **A phantom index entry could reach a caller** (#28): an index row whose backing hash is gone comes back from `FT.SEARCH` as a document id with no fields, and recall built a result from it — empty content, a real key, a slot in `top_k`. `remember()` rejects empty content, so in a memory namespace an empty content field can only mean the record is no longer there; recall now skips and logs those. That settles the open question the issue raised about impact: this was a correctness bug, not only housekeeping
- **`held_back` rule text was cut at 80 characters** (#32), with a hard character slice that landed mid-word: "Extraction-at-promotion keeps the LLM out of the compile path entirely: the in-s". That list exists so a human can decide what to `bless`, the substance of a rule is reliably in the second half of the sentence, and there was no other call that returned the full text — you would have had to `recall_detail()` each source key to reconstruct what you were being asked about. Full text now. The 500-character cap that remains is a safety valve for a pathological rule rather than a display convention: it cuts on a word boundary, appends an ellipsis and sets `truncated: true`, so a trimmed rule can never be mistaken for a complete one. The stored candidate was never clipped, so nothing already blessed carries truncated text. The web UI rendered these semicolon-joined into a single paragraph, which full text makes unreadable — now one per line
- **The staleness clock flagged the memories the system rates most durable** (#34): every entry in the briefing's stale list sat at 179 days and all of them were durable preference facts — preferred git forge, email provider, writing style — stale precisely because they had graduated. Compiled into `preferences-local`, they reach sessions as part of the skill and stop being touched, so the clock inverted and pointed maintenance at the best material in the store. Memories listed in a compiled skill's `source_manifest` are now exempt. The exemption is read live from the manifest rather than stamped onto the memories, so deleting or recompiling a skill takes it with it and no marker ever outlives the skill it refers to. To answer the question the issue raised about severity: nothing archives on the stale list today — auto-archive covers duplicate clusters and expired RSS articles only — so this was misdirection rather than active erosion
- **`reindex()` documented three namespaces and handled four** (#29), and `preference` was neither listed as a valid value nor counted in "all three" despite the unscoped call reindexing it and clearing 34 phantoms from it. It now covers five, `skill` included: that is a real index which `health` already reports on, and rebuilding it is as safe as any other because the SKILL.md body lives in the hash, not the index. The same three-namespace assumption turned up twice more on the grep the issue asked for. `recall()`'s tool description omitted `preference` from the namespaces an agent may search, and `memory_audit()` built its unscoped prefix list by hand and so skipped `mem:preference:` entirely while still accepting `preference` as a filter — an unscoped audit silently under-reported the namespace. Both now derive from the accepted set rather than repeating it
### Added
- **Index drift is reported where someone will see it** (#28): it was only ever a field in `health()`, and nothing prompts an operator to call `health()`, which is how 762 orphaned entries accumulated across four namespaces before anyone looked. The comparison moved to `store.index_report()` so `health()`, `briefing()` and a new startup check share one definition of drift, and it is now logged at boot (`INDEX_DRIFT_CHECK=false` opts out, costs one `SCAN` of `mem:*`) and surfaced in every briefing with the entry count and what to do about it. Deliberately not self-healing: dropping and recreating every index automatically on startup is a much bigger hammer than the problem, and `reindex()` is one call away. Worth recording for anyone reading the issue later — the shipped compose files have set `--notify-keyspace-events AKE` since #11 in April, so disabled notifications are not the explanation for drift on a current deployment
- **`compile_skill()` says why a broad domain compiled nothing** (#33): the `insufficient_reinforcement` response now reports `pool_concentration`, and when every candidate in the pool comes from a single project it says so. That is the whole of the first reading on that issue — for a corpus where nearly every `python` memory belongs to one project, "the Python domain" and "that project's history" are the same set of memories, and reinforcement cannot tell a recurring language pattern from two unrelated episodes in the same codebase — available at the point of use instead of requiring an experiment to find out. The note no longer suggests lowering `min_reinforcement`: when the candidates are narrative of what happened rather than generalisable claims, a lower gate admits the noise instead of finding the signal, turning a domain that correctly compiled nothing into one that compiles the wrong thing. It points at `bless()` and at narrower domains instead. The schema question the issue raises — whether `record_experience()` should prompt for the generalisable lesson separately from the narrative — is left open deliberately; it is a schema change and belongs in a minor release, not a patch
- **Test suite grew to 1913 tests.** `FakeEmbedder` seeded its per-word RNG on `hash()`, which Python randomises per process, so the same word embedded to a different vector on every run and the similarity between any two texts moved between runs. That was invisible while nothing compared a similarity against a threshold and became immediate flakiness once the two floors did; it now seeds on `crc32`. One test keeps a `no_relevance_floor` fixture — `"A"*50` against `"A"*500` is two distinct tokens with no overlap, near zero in the fake and only 0.265 on the real model, and its subject is snippet shape rather than whether the pair matches
- **Two defects in the first cut of these fixes, found in review before release and fixed with tests that fail without them.** The floor could delete a result that matched perfectly: when an extracted fact outranks the verbatim memory it came from, the fact is dropped and the source promoted in its place, but only `adjusted_score` was copied across while the floor reads the raw score — so a fact matching at 1.0 against a source worded differently at 0.39 returned nothing at all. Enrichment routinely distils short facts out of long memories, which is exactly that shape. And the phantom guard tested the wrong thing: "empty content means the record is gone" holds where `remember()` rejects empty content, but `compile_project_context(auto_save=True)` writes `content=""` whenever a project has no description yet and keeps its real text in `current_state` — so a fully populated project context, the one the documented session-start bootstrap creates, became permanently unrecallable and the log told the operator to run `reindex()`, which cannot help. A phantom is now identified by carrying no fields at all beyond the key and score
- **Positive and negative drift are described separately.** One sentence was calling both "index entries without a backing record". They are different faults with different remedies — `reindex()` clears orphans and does nothing for an index that is merely behind — so a shared `drift_note()` says which is which, and the startup check no longer warns when the index is lagging, which is the expected state moments after `_migrate_indexes()` recreates one

## [6.7.0] - 2026-09-08
### Changed
- **Embeddings run on ONNX Runtime; PyTorch is out of the images**: `memory/onnx_embedding.py` runs the model maintainers' own ONNX export of all-MiniLM-L6-v2 (`onnx/model.onnx` in the Hugging Face repo) with `onnxruntime`, tokenises with the Rust `tokenizers` library, and does the mean pooling and L2 normalisation in numpy. It produces the same vectors sentence-transformers did — the benchmark below measured cosine 1.0000 on every sampled text and an identical top-10 on every query — so nothing stored needs re-embedding and no threshold moves. The `Embedder` singleton grew an `EMBEDDING_BACKEND` switch (`onnx`, the default, or `torch`) and the RSS worker follows it; `sentence-transformers` and PyTorch are no longer installed in any image, and `requirements-torch.txt` brings them back for anyone who sets `torch`. The RSS worker image now ships the shared `memory/` package (built from the repo root, as the web UI already was) and calls the same `build_model()` as the server, probed once at boot so a bad configuration stops the container with the reason instead of failing quietly inside every feed cycle. The engine reads the model's own pooling mode from `1_Pooling/config.json` (mean, CLS and max; anything else is refused at start, because a CLS-pooled model mean-pooled would land every new vector in a different space from the stored ones without a word), selects the token-embedding output by name and checks its shape, bounds the token cap to the graph's position table, refuses a model whose vectors don't fit the 384-dimensional indexes, looks in the Hugging Face cache before touching the network (a warm start makes no round trips), pins the default model to the commit the numbers below were measured against so an upstream re-export can't move vectors under a live store, accepts a local directory as `EMBEDDING_MODEL` for air-gapped hosts, and turns a missing ONNX export into an error that names the torch fallback and the offline pre-fetch. The images set `HF_HOME=/app/hf-cache` and compose mounts one shared `hf_cache` volume on it, so a recreated container starts from cache
- **What that bought, measured with `scripts/embedding_bench.py` on an arm64 box, 400-memory corpus, 44 queries**: model load 7.8 s → 0.8 s (−90%; torch varied between 2.6 s and 7.8 s across runs with the disk cache, ONNX Runtime sat at 0.8 s); peak resident memory 924 MB → 301 MB (−67%); `embed()` p50 22.3 ms → 7.9 ms (−64%), p95 40 ms → 17 ms; `remember()` p50 25 ms → 11 ms (−57%); `remember_document()` 82 → 34 ms per chunk (−59%); `recall()` p50 35 ms → 23 ms (−35%), p95 56 → 42 ms; single-text throughput 41 → 119 texts/s (+193%), batch-of-32 93 → 107 texts/s. Equivalence: vector cosine mean 1.0, min 1.0 over 48 texts; top-10 Jaccard 1.0 over the 43 distinct queries, top-1 identical on all of them, score drift 0
- **Four new knobs, all optional**: `EMBEDDING_ONNX_FILE` picks a different graph from the repo (the optimised `onnx/model_O4.onnx`, or quantised `onnx/model_qint8_arm64.onnx` / `onnx/model_qint8_avx512.onnx` — faster and smaller but *not* vector-equivalent to the float model, so benchmark and re-embed before switching), `EMBEDDING_MAX_SEQ_LENGTH` overrides the model's token cap (read from its `sentence_bert_config.json` by default, clamped to the graph's positions), `EMBEDDING_THREADS` pins ONNX Runtime's intra-op threads (the worker's compose entry sets it to 1), and `EMBEDDING_MODEL_REVISION` pins the repo revision. A bare `EMBEDDING_MODEL` name is looked up under `sentence-transformers/`; give `owner/name` for anything else, or a local directory holding the repo's files for an air-gapped host (`HF_HUB_OFFLINE=1` with a pre-filled cache works too). Downloads land in the same Hugging Face cache sentence-transformers used, so an existing cache gains one 86 MB file. The RSS worker's compose entry sets `EMBEDDING_THREADS=1` — `OMP_NUM_THREADS` only ever throttled torch
- **Test suite grew to 1849 tests**, `memory/` and `tools/` at 100% line coverage; the real-model equivalence test (`OMNIMEM_REAL_EMBED_TESTS=1`) asserts the ONNX vectors against reference components from sentence-transformers
- **The Docker images no longer install torch or OpenBLAS**, and the builder stage no longer needs `libopenblas-dev`. The MCP server image went from 2.04 GB (6.6.2) to 634 MB on arm64. Debian-slim stays as the base: the reason for it (PyTorch has no musllinux wheels) is gone, but onnxruntime and tokenizers ship manylinux wheels too and there is nothing to gain from fighting musl
### Added
- **An embedding benchmark for the backend swap**: `scripts/embedding_bench.py` ingests a corpus (a seeded synthetic one, or the text of a real backup), runs a fixed query set, and reports model load time, peak RSS, `embed()` and `embed_batch()` speed, `remember()`, `remember_document()`, `recall()` and `recall_index()` latency — all against a real valkey-search, using whatever backend `memory/embedder.py` is wired to. It also saves the raw vectors for a sample of texts and the top-10 per query, so `compare before.json after.json` reports per-text cosine between the two backends, top-k overlap and score drift: the check that says whether an ONNX export is the same model or merely a faster one. Refuses a populated store, cleans up after itself, exits non-zero on a warning so it can gate CI, and runs against the in-memory fakes in the suite so it cannot rot between the before and after runs. See `docs/embedding-benchmark.md`

## [6.6.2] - 2026-09-07
### Added
- **Every memory now says who is speaking**: a `provenance` field on every writable namespace with three values — `asserted` (the human stated it: a preference, a project description, a fact they gave), `concluded` (the system's own reasoning or write-up of work done), and `retrieved` (an external source: an article, documentation, a page summary). Without it, an inference the system made last month is recalled today and read as independent corroboration for the reasoning that produced it, so errors compound with no external correction; with it, a later session can tell evidence from inference. A buyer of a curated store in a regulated domain is buying exactly this traceability. Set at write time: `remember()` and `remember_document()` take a `provenance` argument (defaulting to `concluded` for episodic and project memories, `asserted` for preferences, `retrieved` for knowledge), project context set by hand is `asserted` (a first draft from `compile_project_context(auto_save=True)` is the system's synthesis and is `concluded`; a recompile carries forward whatever the entry already had, so a human's vouch survives), the RSS worker stamps every article `retrieved`, and an extracted fact inherits its source's class — extraction is restatement, not reasoning, so a fact of something the human asserted is still asserted. The write that queues enrichment carries its licence and provenance in the job payload, so the facts inherit them even if the source is deleted before the worker gets to it. There is no "unknown" provenance: a value is one of the three or not given, and the agent's instructions tell it to pass `asserted` when the human dictated content and `retrieved` when storing what a document said
- **Reported on recall, corrected with `set_provenance`**: every classified result from `recall()`, `recall_index()`, `recall_detail()` and `recent_knowledge()` carries its `provenance`. `set_provenance(provenance, keys)` reclassifies — most often to mark a memory the human vouches for as `asserted` — and facts extracted from a reclassified memory follow it. The web UI's memories page filters on provenance, the detail page shows it with a form beside the licence one, and the create form takes a choice
### Changed
- **The backfill makes the uncomfortable call the roadmap asked for**: a startup migration (rerun after a backup restore) stamps RSS articles `retrieved`, preferences and project context entries `asserted`, extracted facts — in the knowledge and preference namespaces alike — with their source's class, exactly as live enrichment does, and everything else — every existing episodic memory included — `concluded`. An episodic memory is the agent's write-up of work that really happened, so calling the lot "concluded" undersells the most valuable material in the store; but it is honest about who wrote it, the alternative was a guess in the other direction, and it is the class a buyer scrutinises first. The decision is made deliberately rather than left to a migration default, and any memory the human vouches for is one `set_provenance(..., "asserted")` away
- **Never scored on**: recall carries `provenance` through untouched and no multiplier reads it, for the same reason as `licence` — landing a field and changing what recall does with it are separate releases so a benchmark move can be attributed. The suite asserts that two identical memories with different provenance rank identically
- **One engine behind both classification tools**: `set_licence` (both the key and the feed path), `set_provenance` and the two web forms all go through a shared lineage engine that stamps the memories, cascades to their extracted facts (by `enriched_from` and by `source_doc_id`, so batch-enriched documents are covered), treats a chunked document as one rights unit (classifying one chunk classifies its siblings, so the facts and the chunks can never disagree), skips the fact scan entirely when only articles are being classified (they are never enriched), and deliberately leaves `updated_at` alone — the tools and routes in front of it refuse skills and non-memory keys before anything reaches it — the skill compiler reads a bumped timestamp as "source changed". The vocabulary lives in one constant because the v7 Mycelium `ClusterSummary` carries a `provenance_class` that must use the same values; a mismatch there would be silent until cluster time
- **Reads never depend on backfill timing**: recall, `recall_detail`, `recent_knowledge`, the briefing's new-knowledge list and `explain_memory` and the web UI's memories filter resolve a missing `provenance` or `licence` at read time with exactly the rules the backfill writes — an article is `retrieved`/`unknown`, a preference or context entry `asserted`, a fact whose source can't be consulted `concluded`/`unknown`, a legacy plain knowledge write `concluded` — and the suite asserts every shape reads the same before and after the backfill runs. So a record stamped by a worker image that predates the field during a rolling upgrade still reports something honest, an unclassified article still raises the `licence_notice`, and `/memories?licence=unknown` is the complete classify queue before the next restart. An out-of-vocabulary stored value reads as absent, and skill import falls back to the namespace default for one, so a filter can never fail to match what recall reports. Imported memories never keep the exporter's `own`: open and restricted travel with their note, everything else is `unknown`
- **Indexed and returned from day one**: `provenance` is a TAG field on all four writable indexes (the startup migration rebuilds them, data-safe — verified against a real valkey-search, including a combined `@licence:{unknown} @provenance:{retrieved}` filter) and in every namespace's search return whitelist
- **Test suite grew to 1781 tests**, with every `memory/` and `tools/` module at 100% line coverage

## [6.6.1] - 2026-09-07
### Added
- **Every memory now says whether it may be redistributed**: a `licence` field on every writable namespace, set the moment a record is written and never derived later. Four classes: `own` (written here — a decision, a fix, a preference), `open` (third-party content under terms that permit redistribution: OGL, CC BY, CC0, public domain, permissive code licences), `restricted` (third-party content you may not redistribute: paywalled, all rights reserved, and the non-commercial or no-derivatives Creative Commons variants, because a summary is a derivative and a sold bundle is commercial use), and `unknown` (nobody has said yet). A recognised identifier — `ogl-3.0`, `cc-by-4.0`, `all-rights-reserved` — resolves to its class and is kept as a `licence_note`, so a buyer can see *which* open licence applies and not just that one does. Unrecognised identifiers are rejected by the tools and the web UI rather than guessed at (the worker, which cannot bounce a hand-edited feeds.yml back to anyone, logs the value and ingests as `unknown`): a misclassified record is exactly the liability the field exists to prevent
- **Decided at ingest, because afterwards is too late**: the RSS worker stamps every article from its feed's new `licence:` declaration in feeds.yml (or the Licence field in the web UI's feed editor). The worker accrues articles every night, and the ones that cannot be shipped are impossible to pick out cheaply once they have piled up — so the answer is recorded while the source is still in front of you. A feed that declares nothing ingests as `unknown`; with `RSS_REQUIRE_LICENCE=true` such a feed is refused outright, skipped before any fetch and counted under `refused` in the ingest stats, for stores that must never accrue unvetted records. `remember()` and `remember_document()` take a `licence` argument (defaulting to `own` for episodic, project and preference writes, and `unknown` for knowledge — the one namespace that routinely holds third-party material); project context is always `own`; and an extracted fact inherits its source's licence, because a derivative has exactly its source's rights
- **Recall asks you to classify what nobody has**: every classified recall result carries its `licence`, and when any result is still `unknown`, `recall()` appends a `licence_notice` listing the keys (`recall_index()` reports the same under `licence_notice`). The agent's instructions tell it to raise those with the human while the content is on screen and record the answer with the new `set_licence` tool — `set_licence(keys=[...], licence="open")` for specific records, or `set_licence(feed_name="GOV.UK", licence="ogl-3.0")` to classify every article already stored from one feed. Reclassifying clears any note that came with the old value, so a record marked restricted never keeps a stale "CC BY 4.0" beside it (the web UI forms apply the same rule to a pre-filled note when the class changes), and the facts extracted from a classified memory follow it automatically — a derivative has exactly its source's rights, so a source answered as open must not leave its facts nagging as unknown. Classification is metadata, not a content edit, so it never bumps `updated_at`: the skill compiler reads that timestamp as "source changed" and would otherwise raise a high-risk update alert for every skill the memory feeds. `recent_knowledge(licence="unknown")` lists what still needs doing, and the briefing's new-knowledge list shows each article's class
- **The web UI classifies too**: the memories page filters on licence (`/memories?licence=unknown` is the queue), rows nobody has classified wear an **unclassified** pill, every memory's detail page has a redistribution-rights form (class plus optional note, same validation as the tool), the create form takes a licence choice, and the feed editor declares a feed's licence with the feeds list showing each feed's state. Skill bundles carry a feed's declared licence in `feeds.json`, validated on import, so a feed added on the receiving instance ingests classified from its first cycle
### Changed
- **An upgrade backfills with honest unknowns, not optimistic guesses**: a startup migration stamps conversation-sourced namespaces (episodic, project, preference) as `own`, RSS articles and knowledge with no traceable origin as `unknown`, and lets extracted facts inherit from the memory they were extracted from (a fact whose source is gone is unknown — there is nothing left to inherit). Memories that arrived in a skill bundle from another instance (`imported_at`) are someone else's work and become `unknown`, not `own`. It only ever touches records with no `licence` field at all, so a value a human has set is never revisited, writes are batched per value, and it runs again after a backup restore so a pre-6.6.1 dump doesn't leave records in an invisible third state until the next restart
- **The field is reported, never scored on**: recall carries `licence` through untouched and no multiplier reads it. Landing the field and changing what recall does with it are deliberately separate releases — the lesson from the issue #20 fact-extraction regression, where two changes shipped together and the benchmark move could not be attributed
- **It answers one question only**: redistribution *rights*, not disclosure *scope*. Who may see a memory is a different axis with different values, and a summary of a paywalled standard can legitimately be visible to everyone on an instance and still be non-redistributable. Both the docs and the module say so explicitly, because the first person to read `public` as permission to ship is the failure the field exists to prevent
- **Indexed and returned from day one**: `licence` is a TAG field on all four writable indexes (the startup index migration rebuilds them, data-safe) and is in every namespace's search return whitelist, so a filtered recall can never silently drop it the way the knowledge namespace once dropped `project`. The identifier alias table lives in `memory/licence.py` and is copied verbatim into the RSS worker, which does not ship that package; the test suite asserts the two tables are identical so they cannot drift. A feed's declaration only reaches the `meta:feed:influence` mirror (and from there skill bundles) when it resolves, so a typo in feeds.yml is logged locally rather than breaking bundle import on another instance, and a YAML boolean (`licence: no`) is treated as a mistyped declaration, never as an absent one
- **Test suite grew to 1694 tests**, with `memory/` and `tools/` at 100% line coverage

## [6.6.0] - 2026-08-20
### Added
- **Projects declare what kind of work is in them, and recall can search by it**: a project context now carries a list of work-type domains — `python`, `docker`, `wcag-accessibility` — and `recall(query, domain_filter="python")` searches every project declaring that domain at once instead of one project at a time. This is the gap compiled skills left open: a skill only exists once its lessons clear the reinforcement gate, so everything below that threshold, the gotcha you have hit twice and never written up, was only reachable by searching the entire store unfiltered. The domain filter reaches those raw memories. `project_filter` and `domain_filter` intersect when both are given, `list_projects(domain="python")` narrows the project list, and `recall_index()` reports which projects a domain resolved to
- **One vocabulary, not two**: project domains normalise through the *same* code as compiled skill domains, so `py` on a project and `py` on a skill both become `python` and the two systems cannot drift into parallel taxonomies. The same name reaches `find_skills()` and `get_skill()`, and the auto skill scan now ranks a domain higher when projects declare it — a domain two projects have named is exactly the cross-project signal that scan already prefers. The boost only re-ranks candidates that already have lessons behind them; a domain with an empty pool would be dropped at the min-pool gate anyway, so it never spends one of the capped candidate slots
- **Domains suggest themselves**: `compile_project_domains(name)` reads the project's existing `stack` field and the tags that recur across its own memories, and proposes a domain list with the evidence behind every entry — "stack", or "tagged on 4 memories". It proposes by default and only writes with `auto_save=True`, it never removes a domain you set by hand, and a one-off tag is not evidence (a tag has to recur before it counts). `compile_project_context()` includes the same suggestion in its draft, and the web UI's project edit page has a **Suggest from stack & memories** button that fills the field in without saving anything
- **The projects page filters by domain**: domains render as pills on both the projects table and the project detail view, a chip bar above the table filters the list with a count per domain, and a domain that already has a compiled skill links straight to it. The project edit and create forms take a comma-separated domain list with a datalist of every domain already in use across projects and skills, and aliases resolve on save
### Changed
- **The recall project filter takes a set of projects, not just one**: `project_filter` now accepts a list, and the FT.SEARCH push-down composes it with clause-level OR (`(@project:{a} | @project:{b})`) rather than in-brace alternation, which returns an empty result set on valkey-search. A single unsafe value drops push-down for the whole clause rather than filtering on part of the set, and the KNN candidate budget widens as the project set grows so a six-project domain doesn't starve itself
- **An upgrade doesn't leave the feature empty**: a startup migration seeds domains from each project's existing `stack` string, so the domain filter has something to match on the first run rather than silently returning nothing — the same invisible failure shape as the issue #20 whitelist bug. It only ever writes to projects that have never had the field, never overwrites a domain, and records an empty marker when a stack yields nothing usable so it doesn't rescan. Everything it derives is editable in the web UI or replaceable with `compile_project_domains()`
- **A domain nobody declares is reported, never quietly ignored**: filtering on an undeclared domain runs the search unscoped and prepends a `domain_filter_notice` result saying the filter was not applied, because a global search presented as a scoped one is worse than no filter at all. When a domain filter and a project filter are both given and nothing satisfies both, the result is empty plus that notice — the empty scope short-circuits rather than falling through into a search of everything
- **Test suite grew to 1544 tests**, with `memory/` and `tools/` at 100% line coverage
### Fixed
- **Index migration on startup has never actually worked, and now does**: `_migrate_indexes()` compares each index's field count against its definition and drops any index that is short a field so it gets recreated with the new ones. The drop used valkey-py's `client.ft(name).dropindex()`, which appends its delete-documents flag as a positional argument even when it is False — so the command on the wire was `FT.DROPINDEX <index> ""`, three arguments where valkey-search accepts two. RediSearch tolerates the trailing empty string; valkey-search rejects it with "wrong number of arguments", and that error was being caught by the same `except` clause that handles the ordinary case of an index not existing yet. The result was a migration that failed silently on every upgrade since the function was introduced, leaving upgraded instances indexing an older field set while a fresh install got the full one, with nothing in the logs to say so. Any field added to an index in a past release — the knowledge namespace's `project` tag among them — was therefore only ever searchable on fresh installs. The drop now issues the raw two-argument command, a genuine failure is logged as an error instead of swallowed, and the affected indexes rebuild themselves on the next start after upgrading. Nothing is lost when they do: dropping an index removes only the index, and the search module re-indexes the existing hashes automatically. `reindex_namespace()` always used the raw command and was never affected
- **The test fake stopped lying about it**: the in-memory `dropindex()` accepted the broken call happily, which is why the migration tests passed for as long as the bug existed. It now raises the same error a real valkey-search returns, so this class of failure cannot pass the suite again

## [6.5.1] - 2026-07-21
### Fixed
- **The RSS feeds table fits on an iPad again**: the new Skills column made the feeds table six columns wide, but the fixed column widths still summed to 100% across the original five, so the Actions column had no room and the table pushed past the edge of the screen on tablet-width viewports. The widths are rebalanced for six columns, long skill mappings wrap inside their cell instead of forcing the table wider, and the nowrap that had drifted onto the Skills column is back on the Mode column where it belongs. The action cells also stopped using `display:flex` on the `<td>` itself — flex detaches a cell from table layout entirely, so its column width never applied and the buttons spilled out of a collapsed cell; the buttons now flow inline, sitting side by side on desktop and wrapping on narrow screens
- **No page scrolls sideways any more, down to phone width**: a headless audit of all 21 web UI pages at 390, 768, 1024, and 1400px — seeded with long URLs, long content, and long skill mappings — found horizontal overflow on ten of them, from four distinct causes, all fixed. The small-screen media query had been switching the memories, projects, skills, and graveyard tables to `table-layout:auto`, which lets nowrap cells push a table far past the viewport (the memories page ran 490px over on iPad portrait); those overrides are gone and the fixed percentage layouts now hold at every width, with tighter cell padding buying the narrow columns some room. The content column gained `min-width:0` — as a flex item it refused to shrink below the width of one long unbroken URL, which silently widened every page including at 1024px. Page headers now wrap, so action buttons drop below the title instead of forcing the width on telemetry, token overhead, and the skill detail page. And the three tables that had no width rules at all — the experience page's effortful table, backups, and the skill detail's rules table — got fixed layouts of their own. On phones, namespace pills and dates wrap within their narrow columns, column headers ellipsise rather than breaking mid-word, and table buttons slim down

## [6.5.0] - 2026-07-20
### Added
- **RSS feeds can influence compiled skills**: a feed can now be tied to one or more skill domains with an influence score from 1 to 10 — set per feed in the web UI's feed editor (with a suggestion list of existing skill domains) or as a `skills:` mapping in feeds.yml. When such a skill is recompiled, the latest articles from its influencing feeds are pulled into a new **Feed watch** section automatically, no per-article blessing required: the score is how many of its most recent articles a feed contributes, so a 10 dominates the section and a 1 adds a single headline (whole section capped by `SKILL_FEED_MAX_ARTICLES`, default 25, 0 disables). The write gate is untouched — feed items only land through a proposed and accepted compile, the section is labelled as unvetted current signal, and feed-item churn is classified low-risk in the change summary so article rotation never drowns out real rule edits. Feeds without an association keep the existing behaviour: their articles never touch a skill unless a human promotes one. Feeds don't bootstrap skills either — a domain with no experience and no promoted references still compiles nothing from feed association alone. The config mirrors into Valkey (`meta:feed:influence`) on every web UI feed save and on each worker ingest cycle, so hand edits to feeds.yml converge too
- **Skill bundles carry their feeds**: exporting a skill now includes the RSS feeds that influence its domain (name, URL, topics, mode, project label, and the influence score) as a checksummed `feeds.json` in the bundle — export format version 2, with version 1 bundles still importable. Import previews the feed plan alongside the memory plan and stays strictly additive: a feed missing from the receiving reading list is added (renamed with an "(imported)" suffix if its name is taken by a different URL), a feed already present at most gains the influence entry it lacked, and nothing existing — topics, mode, or an existing score — is ever rewritten

## [6.4.2] - 2026-07-19
### Changed
- **The articles page shows the feed, not the project**: every RSS article carries the same project label, so the Project column on the articles view said nothing useful. It now shows which feed each article came from — the `feed_name` the ingester stamps on every article, including per-feed overrides from feeds.yml. Learned knowledge and the plain memories views keep their project column
- **Articles list in the order they arrived**: articles never change after ingestion, but startup migrations and backfills bump `updated_at`, which shuffled the listing. The articles view now sorts and dates rows by `created_at` — the column header reads Added — with a fallback to `updated_at` for anything predating the field. The Newest/Oldest toggle still works, and every other memories view keeps its updated-at ordering
### Fixed
- **The skills table stays inside the page**: the compiled skills table used fixed column widths, but the domain and state badges refuse to wrap and the Export/Delete cell was pinned to one line, so a long domain name ran underneath the description, the state pill ran under the compiled date, and below about 1140px wide the buttons pushed past the edge of the page. Cells now clip instead of spilling, domain and state badges wrap within their columns (kebab-case domains break at their hyphens), the action buttons right-align and stack when the viewport narrows, and the column widths are rebalanced to give the actions their room. A stray duplicate width rule that had drifted into the graveyard table's block is gone too

## [6.4.1] - 2026-07-19
### Added
- **Skills propose themselves now**: at most once a day (`SKILL_SCAN_INTERVAL_HOURS`, 0 disables), a `briefing()` runs an auto skill scan across all projects. Domains with no compiled skill whose lessons already clear the reinforcement gate get a draft proposed automatically — by default only when a rule spans two or more projects, since a lesson that recurs across projects is the strongest signal it deserves to become policy (`SKILL_SCAN_CROSS_PROJECT=false` relaxes that). Skills whose sources have changed get a fresh draft compiled so the diff is ready to review. The scan only ever creates proposal stashes — the same thing `compile_skill(mode="propose")` creates — so the propose-and-accept gate is untouched and a human still commits every draft; results arrive in the briefing's `auto_proposed_skills` section and show up as pending proposals on the web UI's skills page. Ignoring a draft declines it: a per-domain marker remembers the proposed body, so the identical draft is never raised twice — it only comes back when the underlying lessons change. Bounded by `SKILL_SCAN_MIN_POOL` (default 3 lesson-bearing memories before a domain is even checked) and `SKILL_SCAN_MAX_PROPOSALS` (default 3 per run)
### Changed
- **Test coverage rose from 90% to 99.9%**: the suite grew to 1291 tests and every application module now sits at 100% line coverage — the recall pipeline's filter and fallback branches, the maintenance and contradiction scanners, skill transfer's validation edge cases, every web UI route's error paths, the OAuth provider and storage, and the RSS ingester. The MCP server entry point, its on-connect instructions, and the RSS worker's scheduler had never been imported by a test at all; they now have full suites covering the Host/Origin allowlist derivation, the fail-closed public-bind check, both `__main__` blocks, and the feeds.yml watcher. The pass also caught tests that were passing while asserting nothing: the `remember()` contradiction test called with `force=True`, which skips the contradiction check entirely, and three contradiction-scan tests used wording no negation pattern matches — all now assert unconditionally and are deterministic regardless of Python's hash seed
### Fixed
- **One corrupt timestamp no longer takes down the memory list**: a memory with a malformed `updated_at` field used to 500 the whole /memories page; it now sorts as oldest and the page renders
- **`install.sh` no longer hangs under `curl | bash`**: the installer re-execed stdin to the terminal when piped, but under `curl | bash` stdin is the script stream itself, so bash started reading the rest of the script from the keyboard and appeared to hang. Prompts now read from the terminal directly when one is attached and fall back to sensible defaults when not, and the run shows numbered step-by-step progress so a slow image pull doesn't look like a freeze
...
- **Composition bar on the dashboard**: the corpus total is now the hero — a large mono figure beside a full-width bar whose segments are proportional to each namespace's share, in the same colours as the namespace dots in every table. The old Total card is retired; the per-namespace cards remain as the click-throughs
- **Recall heat on memory rows**: each row on the memories views carries a thin left rule that fades with time since the memory last surfaced in recall — bright orange within a week, dimming through a month, gone after ninety days — so a glance shows which memories are alive in the ranking and which are fading. Deprioritised rows also drop their content ink one step (still WCAG AA). The Updated column carries the same information as text, so nothing depends on the colour alone
### Changed
- **The web UI wears the SquareCows palette**: surfaces moved from neutral blue-grey to the brand navy `#00101D` (dark) and warm paper `#FBF7F0` (light), and every accent that was Tailwind indigo is now brand orange `#FF8E01`. The brand's own accessibility rule is enforced throughout: orange text is fine on navy (8.35:1) but fails on paper, so the light theme's accent-coloured text drops to a burnt orange (5.1:1), and anything written on an orange fill — buttons, the version badge, the current page number — is navy, never white. Status colours darken in the light theme to stay AA on paper
- **Namespace pills became dots**: tables carried a pill for namespace, state, and rule kind, all in the same style, so nothing ranked above anything else. Namespaces are now a small coloured dot beside plain text — matching the composition bar's legend — and pills are reserved for lifecycle state, the badge you act on
- **Type has a hierarchy now**: page titles grew to 1.75rem, stat figures are set in Ubuntu Mono 700 (machine numbers in a machine face), and the uppercase-microlabel treatment retreated from form and metadata labels back to table headers and nav section labels, where it earns its keep
- **Empty states direct instead of mope**: no more italic "No memories stored yet." — empty views now say what to do next, with a link to do it
- **Sidebar structure calmed down**: section labels no longer double up with divider lines, and the active item's marker moved from the far right edge to a left rule beside the text
- **A favicon at last**: the logo's `<>` brackets, orange on navy, as an inline SVG

## [6.2.0] - 2026-07-12
### Added
- **Knowledge articles can feed skills — deliberately**: `promote_knowledge(key, domain="python")` marks an article skill-eligible, and the next `compile_skill()` for that domain renders it in a new Reference section — one rule per promoted article, citing the knowledge key and source URL, kept visibly separate from the rules earned through experience. Promotion is the vetting step: an article carries no effort or outcome signal, so a human marking it eligible is what substitutes for reinforcement (the same logic that lets `bless()` carry a single strong lesson past the threshold). The propose-and-accept gate still applies to every compile, promotion clears the article's expiry so it can't auto-archive underneath the skill, and `demote=True` reverses it — the next recompile then flags the dropped Reference rule as a high-risk removal. A domain with promoted articles but no episodic pool compiles a pure-reference skill, so a curated spec can become loadable before any experience accumulates
- **Knowledge watch in the briefing**: the RSS feed now acts as an early-warning system for compiled skills. `briefing()` gains a `skill_knowledge_watch` section comparing recent articles (stored vectors, nothing re-embedded on the hot path) against each skill's discovery embedding and surfacing the ones that look relevant — upgraded to `possible_contradiction` when the tier-1 negation heuristic fires between the article and one of the skill's rules, meaning the world may have moved under a rule. Awareness only: nothing changes a skill without the gate; promote the article or let it age out of the watch window (`SKILL_KNOWLEDGE_WATCH_DAYS`, default 14; similarity floor `SKILL_KNOWLEDGE_WATCH_THRESHOLD`, default 0.35; set days to 0 to disable)
...
- **Delete skills from the web UI**: each row on `/skills` gains a Delete button with a confirmation warning. The source memories survive, so recompiling the domain can recreate the skill. Editing remains impossible by design — a skill's content only changes through a reviewed recompile
- **Outcome filter and pagination on Most Effortful Memories**: the experience page table now pages through every memory with experience data (10 per page) instead of stopping at the top 10, with a selector to show only succeeded, pivoted, or abandoned outcomes. Filter and page swap in place via htmx and survive in the URL
- **Reset button for the tool usage counters**: the token overhead page can now flush the measured per-tool metrics (the `meta:tool_metrics:*` hashes in Valkey) after a confirmation. The labels no longer claim "since uptime" — the counters live in Valkey with AOF persistence, so they were never per-boot; they now read "all time" until the first reset, then "since <date>" using the stored reset timestamp
- **Articles and Learned Knowledge are separate views**: the sidebar's Knowledge Management group now has Articles (RSS-ingested entries, identified by their `feed_name`) and Learned Knowledge (facts extracted from conversation plus manual `remember()` writes) as distinct filtered views of the memories page, each with its own highlight and heading. A `source=rss|learned` query parameter on `/memories` drives the split and survives filter changes and pagination
- **Deprioritise and Delete buttons on memory rows**: every row on the memories views (including Articles and Learned Knowledge) now has inline actions running the same lifecycle engine as the MCP `deprioritise` and `forget` tools. Deprioritise only shows on active memories; Delete asks for confirmation first. Both return you to the exact view you were on (filters and page intact) via a `next` field that only accepts same-site paths
- **RSS articles are labelled with a project**: every ingested article now carries `project: "RSS"` (override per feed with a `project:` key in feeds.yml), so ingested articles can be separated from knowledge captured in conversation — filter by project on the memories page, or `recall(project="RSS")` to search only articles. A startup migration backfills the label onto existing articles (identified by their `feed_name` field, which extracted facts and `remember()` writes never carry). Real-project-scoped recall is unchanged: articles were already excluded there because they had no project. Projects pages, `list_projects()`, and the dashboard projects card only count `mem:project:*` entries, so the label does not create a pseudo-project anywhere
...
### Changed
- **Dashboard projects card drops the raw record count**: the "N records" subtitle counted `mem:project:*` hashes — an internals number — so the card now shows just the distinct project total with the active / deprioritised / archived breakdown, matching the other cards
...
### Fixed
- **Graveyard table no longer scrolls horizontally**: it used the browser's auto table layout, so a long abandonment reason forced the table wider than the viewport instead of truncating. It now gets the same fixed-layout treatment as the memories and projects tables, with long unbroken names wrapping inside their column and the usual reversion to auto layout on small screens
- **RSS feeds table no longer overflows the page**: its fixed column widths covered four columns but the table has had five since the edit/delete buttons arrived, pushing the Actions column past the container edge. Widths rebalanced across all five
- **Dashboard recent activity table matches the reworked list styling**: it now uses the same fixed column layout as the memories page, truncating long content to one line and splitting the timestamp into date and time spans instead of wrapping a raw `2026-07-11 16:56` mid-string and inflating every row to two lines. The stats cache shape check was extended so cached payloads with the old date format are recomputed rather than KeyErroring the template
...
## [6.0.0] - 2026-07-10
### Added
- **Skill compiler**: turns distilled domain procedure (drawn from experience and graveyard memories) into loadable `SKILL.md` documents an agent can load to work the way you work in a domain (python, rust, technical-blogging, ...). Sits beside `compile_project_context`, which stays the priority for ongoing work — a skill earns its keep on greenfield projects where no project context exists yet and the skill is the only thing carrying your conventions. Four new MCP tools:
  - `compile_skill(domain, mode, min_reinforcement?, include_graveyard?, export_path?, description?)` — compiles the domain's experience pool into a skill. `propose` (default) returns a unified diff against the stored skill (or a full draft if new) plus a risk-classified change summary, and stashes the draft; `write` commits only that stashed, human-accepted draft — there is no silent-commit path. Refuses to overwrite anything not flagged `generated: true`
  - `find_skills(query_or_domain)` — ranked discovery over indexed skill metadata (name, description, domain). Exact domain matches lead, and a hand-authored skill outranks a generated one on the same domain: intent outranks inference
  - `get_skill(skill_id)` — returns the whole skill body intact by key, name, or bare domain. Skills are whole document objects in Valkey (`mem:skill:gen:{domain}-{user}`), deliberately not pushed through the chunk-and-embed path: the agent needs the complete procedure with frontmatter and structure intact
  - `bless(memory_key)` — promotes a single strong lesson to skill-eligible now, bypassing the reinforcement threshold at the next compile
- **Promotion gating**: a single episode is a memory; a pattern across episodes earns a skill rule. Do-rules come from breakthroughs, watch-rules from gotchas, don't-rules from the graveyard; do/watch lessons cluster by embedding similarity (`SKILL_CLUSTER_THRESHOLD`, default 0.80) and graveyard entries group by approach name, with reinforcement counted as distinct source memories. `min_reinforcement` (default 2) is a pre-filter keeping proposals high-signal — the human-accept gate is the real safety net
- **Generated SKILL.md format**: a fixed operating contract (identical across all skills, versioned by `contract_version`, keeps the agent feeding the experience pool so the flywheel turns) plus compiled domain guidance with per-rule reinforcement counts and source-memory citations, a frontmatter source manifest, and a provenance section. Compilation is deterministic — same sources render the same body — so diffs show substance, not noise. The `description` (the load trigger) is human-owned: compiler-drafted in a "Load when: ..." cue format at creation, then pinned so recompiles never clobber it; pass `description=` to change it deliberately
- **Briefing integration**: `briefing()` now returns `skill_suggestions` (a recommendation, never an auto-load — below project context on ongoing projects, moved to the top on greenfield ones) and `skill_updates` (one-line gists where a skill's source memories changed since compile, with prominence scaled to risk: new lessons are low-risk and batch-accept eligible; updated or removed manifest sources are flagged loudly and name the rules they feed). The full diff is always one call away via `compile_skill(mode='propose')`
- **Domain guard**: domains are free-form tags with a "did you mean" guard — a small alias map (`py`→`python`, `golang`→`go`, ...) resolves obvious synonyms, and embedding similarity surfaces near-misses ("looks like `technical-blogging`, use that?") when a domain has no candidates, so scattered tags don't silently starve the reinforcement threshold
- **Skill storage**: new `mem:skill:` namespace with an `idx:skill` vector index over discovery metadata only (the body is never chunked or returned by search). Backups already cover `mem:skill:*`; both restore paths (MCP tool and web UI) now re-embed skills from their discovery text since they carry no `content` field. `export_path` on write optionally mirrors the SKILL.md under `SKILL_EXPORT_DIR` (path-traversal hardened); Valkey stays canonical
### Notes
- Out of scope by design, per the v6 brief: no auto-loading (suggestion only), no auto-write to skills (propose-and-accept only), preferences stay in the preference layer, and no federation (that is v7)
...
- **OAuth / MCP access broke behind a reverse proxy or tunnel after the FastMCP 3.x upgrade**: FastMCP 3.x added `HostOriginGuardMiddleware`, which rejects any request whose `Host` header is outside its allowlist (localhost + bind host) with `421 Misdirected Request`, and any browser `Origin` outside its allowlist with `403 Forbidden Origin`. Behind a proxy/tunnel (Traefik, Caddy, Tailscale funnel) the public hostname isn't allowlisted, so every request 421d — including the `/.well-known/*` OAuth discovery endpoints, making it look like OAuth itself was broken. Once the host was fixed, the browser login POST then 403d because the proxy terminates TLS and forwards over http, so FastMCP's derived origin (`http://host`) never matched the browser's `https://host`. `server.py` now derives both the allowed host and the full `scheme://host[:port]` origin from `OAUTH_BASE_URL` / `MCP_PUBLIC_URL` and writes them into `fastmcp.settings.http_allowed_hosts` / `http_allowed_origins` before `mcp.run()`, so a correctly configured OAuth deployment works with no extra config. Added `MCP_ALLOWED_HOSTS` and `MCP_ALLOWED_ORIGINS` (comma-separated) for extra hostnames/origins not covered by `OAUTH_BASE_URL`. Documented the trap and diagnostics in the README and CLAUDE.md
